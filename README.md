...
脚本会用中文询问端口、转发目标等配置，直接回车使用默认值。安装会同时装好 realm、gost、socat 三种后端，并创建一条默认的 realm 转发规则。
...
--listen-port PORT         默认转发规则的监听端口。默认: 33507
--remote-host HOST         默认转发规则的目标地址。默认: www.mokuoha.com
--remote-port PORT         默认转发规则的目标端口。默认: 33507
